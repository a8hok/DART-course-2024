...
String isValidUser(
  ...
  for (int i = 0; i < users.length; i++) {
    //   print(users[i]['login']['username']);
    //   print(credential);
    //   print(credential['username']);
    if(credential['username'] == users[i]['login']['username']) {
 ...
 return('No user registered so far!');
}
...
void main() {
  ...
  for (int i = 0; i < usersCredentials.length; i++) {
    String loginStatus = isValidUser(usersCredentials[i], users);
    print(loginStatus);
  };
}
